...
def is_title_gradeable(pr_title):
    '''
    ...
    dated_type = sum([bt in pr_title.lower() for bt in badge_types])>0
    if dated_type:
        date_included = bool(date_re.search(pr_title))
    else:
        date_included = True
    return badge_type_included and date_included
...
def process_pr_json(json_output,numbered=False,titles_only=False,
    ...
    json_output : filename
        file generated from `gh pr list -s all --json title,latestReviews ` and optionally 
        additional fields
    numbered : bool
    ...
    custom_field_parser : dict
        dictionary with keys of fields in the json result and values as functions
